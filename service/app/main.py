...
from app.core.model_loader import load_text_embedding_model, load_rqvae_model, ConfigurationError # Updated import
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        raise RuntimeError(f"Failed to initialize models due to an unexpected error: {e}")
...
async def health_check():
    ...
    model_status = "loaded" if hasattr(app.state, 'rqvae_model') and app.state.rqvae_model else "not loaded"
    return {"status": "ok", "message": "Service is running", "rqvae_model_status": model_status}
