...
def save_inter(args, inters):
    ...
    with open(
        os.path.join(args.output_path, args.dataset, f"{args.dataset}.inter"), "w"
    ) as file:
...
def save_feat(args, feat, all_items):
    ...
    num_item = 0
    with open(
        os.path.join(args.output_path, args.dataset, f"{args.dataset}.item"), "w"
    ) as file:
        # "title": title, "description": descriptions, "brand": brand, "categories": categories
        file.write(
            "item_id:token\ttitle:token_seq\tbrand:token\tcategories:token_seq\n"
        )
        for iid in iid_list:
            if iid in all_items:
                num_item += 1
